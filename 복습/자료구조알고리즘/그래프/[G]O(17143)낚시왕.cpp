#include <iostream>
#include <vector>
#include <map>
#include <tuple>

using std::cin;
using std::cout;
using std::vector;
using std::pair;
using std::map;
using std::tuple;
using std::make_tuple;

int R, C, N;
/// N은 상어의 마리 수

class ob {
public:
    int r, c, s, d, z;
    ob(int R = 0, int C = 0, int S = 0, int D = 0, int Z = 0) : r(R), c(C), s(S), d(D), z(Z) {}
};

int RR[5] = {0, -1, 1, 0, 0};
int CC[5] = {0, 0, 0, 1, -1};

void searchAndKill(vector<vector<int>>& s1, int currentHumanColumn, int& sum) {
    for (int j = 0; j < R; j++) {
        if (s1[j][currentHumanColumn] != 0) {
            sum += s1[j][currentHumanColumn];
            s1[j][currentHumanColumn] = 0;
            break;
        }
    }
}

void sol(vector<vector<int>>& s1, vector<ob>& shark) {
    int sum = 0;
    for (int i = 0; i < C; i++) {
        int currentHumanColumn = i;
        searchAndKill(s1, currentHumanColumn, sum);

        vector<vector<int>> newS(R, vector<int>(C, 0));
        vector<ob> newSharks;

        for (auto& sh : shark) {
            auto& [r, c, s, d, z] = sh;

            if (s1[r][c] != z || z == 0) {
                continue;
            }

            int nr = r, nc = c;
            int ns = s % ((d <= 2 ? (R - 1) : (C - 1)) * 2);

            for (int j = 0; j < ns; j++) {
                if (!(nr + RR[d] >= 0 && nr + RR[d] < R)) {
                    d = d == 2 ? 1 : 2;
                }

                if (!(nc + CC[d] >= 0 && nc + CC[d] < C)) {
                    d = d == 3 ? 4 : 3;
                }

                nr = nr + RR[d];
                nc = nc + CC[d];
            }

            if (newS[nr][nc] < z) {
                newS[nr][nc] = z;
                newSharks.push_back(ob(nr, nc, s, d, z));
            }
        }

        s1 = newS;
        shark = newSharks;
    }

    cout << sum;
}

int main(void) {
    cin >> R >> C >> N;

    int r, c, s, d, z;
    vector<vector<int>> s1(R, vector<int>(C, 0));
    vector<ob> shark;

    for (int i = 0; i < N; i++) {
        cin >> r >> c >> s >> d >> z;
        s1[r - 1][c - 1] = z;
        shark.push_back(ob(r - 1, c - 1, s, d, z));
    }

    sol(s1, shark);

    return 0;
}
